  ...
  -webkit-text-fill-color: currentColor !important;
	...
	*/
	...
	const zh = {
		...
		cardTitle: "特效分类",
		master: "全部特效",
		masterHint: "下面所有分类的总开关. 关掉它会强制关闭全部装饰性特效; 加载与进度动画有单独开关, 不受它代管.",
		motion: "动画与过渡",
		motionHint: "关掉后停掉装饰性动画, 并把过渡压缩到 1 毫秒; 加载指示器与进度条由下面那一项单独控制.",
		spinnerMotion: "加载与进度动画",
		spinnerMotionHint: "关掉后进度条与加载指示器的动画也一起停, 界面看起来就像卡住; 它与总开关互不影响.",
		blur: "毛玻璃与背景模糊",
		...
		masterNotice: "总开关已关闭, 下面各分类被强制关闭.",
	...
	const en = {
		...
		master: "Every effect",
		masterHint: "Master switch for the categories below. Turning it off forces every decorative effect off; loading and progress animations have their own switch and are not covered by it.",
		...
		spinnerMotionHint: "Turning this off stops spinners and progress bars too, so a stall looks like a hang. It is independent of the master switch.",
		...
		masterNotice: "The master switch is off, so every category below is forced off.",
	...
	function createReduceEffectsPage(deps) {
		...
		return function ReduceEffectsPage(props) {
			...
			const rows = [row(MASTER_FIELD, "master", "masterHint", masterOn, locked, void 0), ...CATEGORY_FIELDS.map((field) => {
				const forcedOff = !masterOn && !(field === "spinnerMotion");
				return row(field, field, `${field}Hint`, forcedOff ? false : settings[field], locked || forcedOff, forcedOff ? t("masterNotice") : void 0);
			})];
